...
const StationSearch: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const getAllStation = async () => {
            setLoading(false);
            try {
                const response = await axios.get(`https://uts-dev.onrender.com/api/auth/v1/getStation`);
                // console.log(response.data);
                if (response?.data) {
                    setAllStations(response?.data?.map((data: any) => data));
                    setFilteredStations(response?.data?.map((data: any) => data));
                    setLoading(true);
                }
            } catch (error) {
                // console.log(error);
        ...
        if (allStations?.length < 1) getAllStation();
    }, []);
    ...
    useEffect(() => {
        ...
        if (searchTerm) getSearchStation();
    }, [searchTerm]);
    ...
    return (
        <>
            <div className="searchContainer">
                <ProgressBar
                    message="Fetching Your current location..." isLoad={loading} />
                <div className="top">
                    <Link to="/home">
