...
import { Box, Button, Card, Divider, FormControl, FormControlLabel, Grid, Radio, RadioGroup, Typography } from "@mui/material";
...
const BookTicket = () => {
    ...
    const [enCodeUrl, setEncodeUrl] = useState<{ depart: string, goTo: string }>();
                    ...
                    <FormControl className="radioBtn">
                        ...
                        >
                            <FormControlLabel
                                ...
                                value={radioSelectedValue || "paperLess"}
                                control={<Radio sx={{
                                    '& .MuiSvgIcon-root': {
                                        fontSize: 15,
                                    },
                                ...
                                label={<span className="radioLabel" style={{ fontSize: '2.5vw' }}>Book & Travel (Paperless)</span>} />
                            ...
                            <FormControlLabel
                                ...
                                label={<span className="radioLabel" style={{ fontSize: '2.5vw' }}>Book & Print (Paper)</span>} />
                        </RadioGroup>
                    </FormControl>
                </div>
                        ...
                        <Card sx={{ width: "90%", m: 2 }} >
                            {radioSelectedValue === "paperLess" ?
                                (<ul className="disclamer">
                                    ...
                                    <li style={{ color: "red" }}><b>* No cancellation is allowed for paperless ticket.</b></li>
                                </ul>) : radioSelectedValue === "paper" ? (<ul className="disclamer">
                                    <li>* Book anywhere.</li>
                                    <li>* Print ticket at originating station.</li>
                                    <li>* PRINTED TICKET is travel authority.</li>
                                    <li style={{ color: "red" }}><b>* Travel without ticket printout is punishable.</b></li>
                                </ul>) : ""}
                        </Card>
                        {/* <StationSearch /> */}
                    </div>
                ...
                >
                    <Grid container spacing={2} alignItems="center" justifyContent="center">
                        <Grid item xs={5} onClick={() => getSearchPage('source')}>
                            ...
                            <p style={{ textAlign: "center" }}>{paramsObj && paramsObj?.key == "source" ? paramsObj?.code : "STN"}</p>
                            <Typography variant="body2" align="center" sx={{ fontSize: "0.6rem" }}>
                                {paramsObj && paramsObj?.key == "source" ? paramsObj?.name : "Station Name"}
                            </Typography>
                            <Divider sx={{ marginY: 1, backgroundColor: '#FF7F50' }} />
                        </Grid>
                        <Grid item xs={2} sx={{ display: 'flex', justifyContent: 'center', flexDirection: "column" }}>
                            <img src="/arrow_right_uts.png" height={25} width={28} style={{ marginBottom: -10 }} />
                            <img src="/arrow_right_uts.png" height={25} width={28} style={{ rotate: "180deg" }} />
                        </Grid>
                        ...
                        <Grid item xs={5} onClick={() => getSearchPage('destination')} >
                            ...
                            <p style={{ textAlign: "center" }}>{paramsObj && paramsObj?.key == "destination" ? paramsObj?.code : "STN"}</p>
                            <Typography variant="body2" align="center" sx={{ fontSize: "0.6rem" }}>
                                {paramsObj && paramsObj?.key == "destination" ? paramsObj?.name : "Station Name"}
                            </Typography>
                            <Divider sx={{ marginY: 1, backgroundColor: '#FF7F50' }} />
                        </Grid>
                    </Grid>
